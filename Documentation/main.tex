...
\begin{table} [hp]
    \centering
    \caption[Performance metrics Basic]{Performance metrics using the propossed algorithm}\label{table:metrics1}
\csvautobooktabular{../metrics.csv}
\end{table}

For an extra point of camparison, we will be using \textbf{mean squared error} where we obtain a value of \input{../MSE.txt}. Here we are obtaining that we are having a huge error 
